...
import importlib
import json
import inspect  # fork: better-hermeslcm
import logging
...
class BypassMixin:
    ...
    def _get_host_fallback_compressor(self) -> Any:
        ...
        try:
            ...
            #
            # fork: better-hermeslcm — drop only the kwargs this host cannot take. Upstream fell back
    ...
    def _constructor_supported_kwargs(factory: Any, kwargs: Dict[str, Any]) -> Dict[str, Any]:
        """fork: better-hermeslcm — the subset of ``kwargs`` this constructor actually accepts.
    ...
    def _truncate_bypass_content_value(content: Any, char_budget: int, *, suffix: str = "") -> Any:
        """fork: better-hermeslcm — the cut marker is kept even at a zero budget (audit p05 BY01).
    ...
    def _trim_bypass_compacted_to_cap(
        ...
        while len(compacted) > 2 and count_messages_tokens(compacted) > target_tokens:
            ...
            for idx, msg in enumerate(compacted):
                if idx == 0:
                    continue
                # fork: better-hermeslcm — never remove the receipt that says messages were removed
            ...
            if not remove_indices:
                ...
                if remove_index >= len(compacted) - 1:
                    # fork: better-hermeslcm — protecting the receipt must never cost the NEWEST
        ...
        for _ in range(12):
            ...
            for index, msg in enumerate(compacted):
                ...
                if index == newest_index:
                    # fork: better-hermeslcm — the newest message is the request the agent has to
                ...
                # fork: better-hermeslcm — a cut carries a marker (marked_loss.BYPASS_TRIM_SUFFIX)
        ...
        # fork: better-hermeslcm — drop from the front, but never the receipt and never the newest
        ...
        # fork: better-hermeslcm — an ORDER of last resorts, because trimming every message together
    ...
    def _fallback_tail_compaction(
        ...
        *,
        ...
        # fork: better-hermeslcm — the receipt says HOW MUCH went. Upstream's marker named neither the
        ...
        # fork: better-hermeslcm — the cap loop above may remove more messages after the receipt was
    ...
    def _bypass_envelope_chars(message: Dict[str, Any]) -> int:
        """fork: better-hermeslcm — the size of everything a dropped message carried.
    ...
    def _refresh_bypass_receipt(
        ...
        """fork: better-hermeslcm — restate the receipt's counts from the FINAL result.
        ...
        # fork: better-hermeslcm — the receipt is CUMULATIVE. Every surviving receipt was rewritten
    ...
    def _compress_lcm_bypassed_session(
        ...
        *,
        ...
        try:
            ...
            return compacted
        self._mirror_host_fallback_state(compressor)
        # fork: better-hermeslcm — an abort is a decision to PRESERVE, not a failed attempt. Upstream
        ...
        if target_tokens is not None and count_messages_tokens(compacted) > target_tokens:
            if native_aborted and not native_changed:
                # fork: better-hermeslcm — say plainly that the host's preservation decision is being
