...
_V5_CORE_OPTIONAL_COLUMNS: dict[str, frozenset[str]] = {
    # fork: better-hermeslcm — ``envelope_extra`` holds the host fields the columns do not project
...
_KNOWN_FEATURE_TABLE_PREFIXES = (
    ...
    "lcm_query",
    "lcm_trajectory",
    "lcm_node",  # fork: better-hermeslcm — lcm_node_meta sidecar (node_meta.py)
)
...
def _fts_missing_triggers(conn: sqlite3.Connection, spec: ExternalContentFtsSpec) -> bool:
    ...
    fork: better-hermeslcm — the check compared NAMES only, so a trigger of the right name with a
    ...
    with _fts_repair_ownership(conn) as owns_transaction:
        ...
        if degraded:
            ...
            if triggers_were_missing:
                # fork: better-hermeslcm — the trigger SQL is CREATE ... IF NOT EXISTS, so a trigger
