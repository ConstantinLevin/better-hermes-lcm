...
# a COMPLETE numeric lexeme. Without the boundaries, "1e3 USD" offered 1 and
...
def _explicit_numbers(text: str) -> list[int | Decimal]:
    ...
    values.extend(
        ...
        # a whole WORD-number, not a component of one: "twenty-five" offered
...
def _numeric_values_match(parsed: int | Decimal, value: int | float) -> bool:
    ...
    if isinstance(value, int):
        return parsed == value
    # EXACT. An absolute 1e-9 tolerance accepted 0 as the value of a quote
...
def _clause_containing_value(quote: str, value) -> "str | None":
    ...
    presence in the quote is not attribution. "Alice paid 10 USD; Bob paid
...
def _ground_one(
    ...
    *,
    ...
    # presence is not attribution, and a quote can state a value in order to
...
def _format_number(value: int | float | Decimal) -> str:
    ...
    # display rounding must not turn a real quantity into a different one:
...
def verify_final_answer(candidate: Any, trace: ComputationTrace) -> VerificationDecision:
    ...
    # ONLY the canonical answer is verifiable here. Preserving the numbers,
