...
# fork: better-hermes-lcm — a COMPLETE numeric lexeme. Without the boundaries, "1e3 USD" offered 1 and
...
def _explicit_numbers(text: str) -> list[int | Decimal]:
    ...
    values.extend(
        ...
        # fork: better-hermes-lcm — a whole WORD-number, not a component of one: "twenty-five" offered
...
def _numeric_values_match(parsed: int | Decimal, value: int | float) -> bool:
    ...
    if isinstance(value, int):
        return parsed == value
    # fork: better-hermes-lcm — EXACT. An absolute 1e-9 tolerance accepted 0 as the value of a quote
...
def _clause_containing_value(quote: str, value) -> "str | None":
    ...
    fork: better-hermes-lcm — presence in the quote is not attribution. "Alice paid 10 USD; Bob paid
...
def _ground_one(
    ...
    *,
    ...
    # fork: better-hermes-lcm — presence is not attribution, and a quote can state a value in order to
...
def _format_number(value: int | float | Decimal) -> str:
    ...
    # fork: better-hermes-lcm — display rounding must not turn a real quantity into a different one:
...
def verify_final_answer(candidate: Any, trace: ComputationTrace) -> VerificationDecision:
    ...
    # fork: better-hermes-lcm — ONLY the canonical answer is verifiable here. Preserving the numbers,
    ...
    # round-3 verify-2/verify-3). This verifier establishes arithmetic over exact spans; it
    # cannot establish any other proposition, so anything beyond the deterministic answer —
    # however it is punctuated — falls back rather than being certified.
    if _canonical_answer_shape(without_citations) != _canonical_answer_shape(
        re.sub(r"\s*\[lcm:\d+:\d+-\d+\]", "", str(trace.answer or "")).strip()
    ):
        return VerificationDecision(
            "fallback",
            "only the computation's own answer can be verified here; the candidate says more "
            "than the computation establishes",
        )
    return VerificationDecision("verified")


def _canonical_answer_shape(text: str) -> str:
    """Whitespace- and case-insensitive comparison form for the deterministic answer."""
