...
from .errors import ExtractionUnavailableError  # fork: better-hermes-lcm
from .model_routing import apply_lcm_model_route
from .prompt_boundary import build_untrusted_data_messages  # fork: better-hermes-lcm

# fork: better-hermes-lcm — reasons that mean the model was cut off (mirrors escalation).
...
# fork: better-hermes-lcm — no whitespace in the payload class. With ``\s`` in it the match ran past
...
# fork: better-hermes-lcm — the payload stops AT its padding. With "=" inside the repeated class, a
...
# fork: better-hermes-lcm — the instruction half of the extraction prompt, for the trusted system role.
...
def _call_extraction_llm(prompt: "str | list[dict[str, str]]", model: str = "",
    ...
    fork: better-hermes-lcm — a provider failure raises instead of returning ``None``. Returning None
...
def _sanitize_string_media(text: str) -> str:
    ...
    # fork: better-hermes-lcm — how MANY attachments, not merely "some": two inline data URIs in one
...
def _structured_outcome_suffix(block: Dict[str, Any]) -> str:
    """fork: better-hermes-lcm — the outcome fields that sit BESIDE a block's text (audit p05 EX03)."""
    parts: List[str] = []
...
# fork: better-hermes-lcm — keys that carry no content of their own. Everything else is inventoried by
...
def _unrendered_field_receipt(block: Dict[str, Any], rendered_keys, prefix: str = "") -> str:
    """fork: better-hermes-lcm — name substantive fields the rendering does not show.
    ...
    for key, value in block.items():
        ...
        if key in rendered_keys or key in _ACCOUNTED_BLOCK_KEYS:
            continue
        # fork: better-hermes-lcm — 0 and False are VALUES, not absence (round-3 verify-4 #9)
...
def _sanitize_content_block(content: Any) -> str:
    ...
    if isinstance(content, list):
        parts: List[str] = []
        media_count = 0  # fork: better-hermes-lcm — how many, not merely "some" (audit p05 EX03)
        for block in content:
            block_text = _sanitize_content_block(block)
            if not block_text:
                continue
            if block_text.startswith(_MEDIA_ATTACHMENT_MARKER):
                # fork: better-hermes-lcm — a media block can now carry a receipt for the substantive
    ...
    if isinstance(content, dict):
        ...
        if block_type in _TEXT_BLOCK_TYPES:
            ...
            if isinstance(text_value, dict):
                # fork: better-hermes-lcm — a NESTED text object's other fields (annotations, a
            ...
            # fork: better-hermes-lcm — a typed TEXT block can still carry an outcome beside its text
        ...
        # fork: better-hermes-lcm — a block may carry BOTH `text` and `content` (stdout and stderr, for
...
def _injection_marker(removed: str, mark: bool, compact: bool = False) -> str:
    """fork: better-hermes-lcm — one marker for a removed injected block, empty when it held nothing.
...
def _mark_header_removal(text: str, mark: bool, compact: bool) -> str:
    """fork: better-hermes-lcm — the untrusted-context header is removed WITH a receipt when asked.
...
def strip_injected_context_blocks(text: str, *, mark: bool = False, compact: bool = False) -> str:
    ...
    fork: better-hermes-lcm — ``mark=True`` leaves a marker naming how much was removed, for the paths
    ...
    for tag in _INJECTED_CONTEXT_TAGS:
        ...
        # fork: better-hermes-lcm — a self-closing tag carries its content in its ATTRIBUTES
        ...
        while True:
            ...
            # fork: better-hermes-lcm — the removal boundary is upstream's (safety first: a spoofed
            ...
            if closer is None:
                if _at_line_end(cleaned, opener.end()):
                    ...
                    # fork: better-hermes-lcm — an unmatched INLINE opening tag carries its content in
...
def _sanitize_json_like(value: Any) -> Any:
    if isinstance(value, dict):
        # fork: better-hermes-lcm — a sanitised key may never take another key's place. Upstream
        ...
        # fork: better-hermes-lcm — a KEY that was rewritten is a removal like any other, and it left
    ...
    if isinstance(value, str):
        # fork: better-hermes-lcm — a compact marker inside tool ARGUMENTS. The argument block has its
...
def sanitize_pre_compaction_tool_arguments(arguments: Any) -> str:
    ...
    # fork: better-hermes-lcm — duplicate JSON keys are collapsed by json.loads, so re-serialising a
...
def extract_before_compaction(
    ...
    source_store_ids: "List[int] | None" = None,  # fork: better-hermes-lcm (audit p05 EX07)
) -> bool:
    ...
    """
    try:
        # fork: better-hermes-lcm — the same untrusted-data boundary the summariser uses. Upstream
        ...
        # fork: better-hermes-lcm — "the model said there is nothing to extract" and "the route
        ...
        if session_id:
            header += f" ({session_id})"
        header += "\n"
        # fork: better-hermes-lcm — say exactly which rows these bullets came from. A note headed only
        ...
        if source_store_ids:
            ...
            # fork: better-hermes-lcm — the COMPLETE manifest, compressed into ranges. "+N more" and a
