...
def _structured_part_text(part: Dict[str, Any]) -> str:
    """fork: better-hermeslcm — EVERY text-bearing field of the block, not just the first one found.
...
def _structured_part_has_visible_assistant_content(part: Any) -> bool:
    ...
    if part_type in _VISIBLE_TEXT_PART_TYPES:
        if _strip_reasoning_blocks(_structured_part_text(part)).strip():
            return True
        # fork: better-hermeslcm — a typed text block whose text is empty can still carry the thing
...
def _strip_structured_text_part(part: Dict[str, Any]) -> Dict[str, Any] | None:
    """fork: better-hermeslcm — strip EVERY text field of the block, and judge the block afterwards.
...
def _mark_internal_removal(cleaned_content: Any) -> Any:
    """fork: better-hermeslcm — say, in the replay itself, that this turn was cut.
...
def _content_carries_text(value: Any) -> bool:
    ...
    fork: better-hermeslcm — this asked a fixed list of text-bearing keys, so a reasoning block whose
...
def _clean_active_assistant_message(msg: Dict[str, Any]) -> Dict[str, Any] | None:
    ...
    if cleaned_content is None:
        # fork: better-hermeslcm — a turn holding ONLY internal content is not dropped without a
...
def _is_internal_replay_receipt_only(msg: Any) -> bool:
    ...
    fork: better-hermeslcm — such a turn carries no content of its own, so under assembly budget
