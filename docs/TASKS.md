...
|---|---|---|
| M1 | **Semantic conflict, no textual conflict.** Upstream edits a different hunk of a function the fork also edited. Both edits apply; the *combination* is wrong (an upstream early-return placed before our marker; a reordering that makes our guard run after the thing it guards). | Review per FUNCTION, not per hunk: for every symbol carrying a `# fork: better-hermeslcm` marker that upstream also changed, read the merged body end to end and re-derive what it now does. A three-way diff of the hunks is not enough. |
...
2. Compute the **intersection**: which upstream-changed symbols also carry a
   `# fork: better-hermeslcm` marker. That set is the M1/M3/M5 risk surface and gets read function
...
5. `grep -rn "# fork: better-hermeslcm"` and confirm each hook is still true of the merged tree:
