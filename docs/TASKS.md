...
| B5 | CI on the `better-hermes-lcm` branch; the host-integration lane must fail, not skip, when the host import fails (T2.5). |
| B6 | Docs/skill/defaults generated from `ENV_FIELD_SPECS` + anchors so they cannot drift from the curve (T2.4). |
| B7 | **Nothing fails when a fork hook stops being REACHED (catches M2).** Upstream renames, moves or stops calling the function a hook lives in; the fork code still exists, its own unit test still passes, and it never runs. Neither a diff nor a grep for `# fork: better-hermes-lcm` catches that — both see the marker sitting there. What is needed is one check per hook that exercises the real call path and fails when the fork behaviour is absent from it. This is the single highest-value piece of merge insurance the fork does not have, and it is what B12 deliberately did not replace when the touchpoints table went. |
...
These have no completion state. They fire on an external event and each run ends with a
...
|---|---|---|
| M1 | **Semantic conflict, no textual conflict.** Upstream edits a different hunk of a function the fork also edited. Both edits apply; the *combination* is wrong (an upstream early-return placed before our marker; a reordering that makes our guard run after the thing it guards). | Review per FUNCTION, not per hunk: for every symbol carrying a `# fork: better-hermes-lcm` marker that upstream also changed, read the merged body end to end and re-derive what it now does. A three-way diff of the hunks is not enough. |
...
| M5 | **Our fix becomes obsolete or actively wrong.** Upstream restructures the thing we worked around; our patch is now dead weight, or worse, fights the new structure. | Ask it of every hook, not only the conflicting ones: is the thing this worked around still there? Removing a fork patch is a legitimate merge outcome — delete the marker with it. |
...
2. Compute the **intersection**: which upstream-changed symbols also carry a
   `# fork: better-hermes-lcm` marker. That set is the M1/M3/M5 risk surface and gets read function
   by function, in the merged tree, after merging — not as hunks.
...
5. `grep -rn "# fork: better-hermes-lcm"` and confirm each hook is still true of the merged tree:
   it exists, it is still reached, and its reason still applies (M2, M5, M9). Nothing automates
   the "still reached" half — that is B7.
6. `bash scripts/test.sh` green. An upstream test that pins removed behaviour gets re-pointed
   with a `# fork:` comment saying what it used to assert — never deleted silently.
7. Both e2e anchors clean
...
**R3 — Real sessions exist: hunt bugs in the transcripts they left behind.**
Trigger: any Hermes session that ran with LCM enabled. Not applicable yet, and the day it is,
this becomes the most valuable audit the fork has.

**Why it outranks the synthetic evidence.** Everything the fork currently checks itself against
is generated by the fork's own assumptions. `scripts/e2e_no_loss.py` drives a scripted
conversation through a stubbed summariser: it proves the invariants hold for the shapes someone
thought to script, and for no others. The three defects a review found on 2026-09-10 were all
invisible to it — a twice-edited message stranding its original behind the frontier, a tool
result named with `name` instead of `tool_name` archiving a false revision every turn, and a
weight that stops being a weight outside the two anchor windows. None of those shapes occurs in
a scripted run. All three occur in real use.

**What a real `lcm.db` has that nothing else does:** genuine host edits and re-sends, real
spillover markers with real paths, session rebinds and `/new`, tool results in whatever shape the
configured provider actually emits, envelope fields nobody anticipated, and — for a long enough
session — condensation at depth ≥ 1, which the e2e has never once reached.

**Procedure.** Work on a COPY of the database, never the live file, and analyse it
**structurally**: ids, counts, reachability, shapes. It is the owner's actual conversation, so
this audit does not need to read what was said and must not turn into that.

1. Reachability, the invariant the whole fork is for: every stored row is either raw in the
   active context or reachable from a published node. Any row that is neither is a defect, and
   the frontier position at the time is the first thing to look at.
2. Revision chains: every row carrying a supersession key, walked to its original. Every version
   in the chain belongs to some leaf. This is exactly where the 2026-09-10 defect lived.
3. The DAG's actual shape: depths reached, fan-in per node, how much each node stands for,
   whether any node's sources are missing. Compare against what the settings say should have
   happened at that window — a divergence is either a bug or a setting that does not mean what
   it says.
4. Markers: a receipt that claims a removal for a turn that held nothing is a false claim; a
   removal with no receipt is the failure this fork exists to prevent. Both are findable by
   counting.
5. Whatever the host actually sent: which envelope keys appear, which of them survive the round
   trip through `to_openai_msg`, and whether any of them mean something the plugin ignored.
6. Every finding gets reproduced as a probe against a fresh database FIRST — a defect only
   visible in one real database is a lead, not yet a finding — and then fixed on its merits.
