...
# per-character cost for the fallback estimate, in tokens per character.
...
def _serialize_for_count(value) -> str:
    """render a non-string value the way a provider would receive it.
...
def _count_tokens_core(text) -> int:
    if not isinstance(text, str):
        text = _serialize_for_count(text)
    enc = _get_encoder()
    if enc is not None:
        try:
...
def set_token_cache_size(maxsize: int, *, owner: Any = None) -> None:
    """resize the memo (window-weighted: 2048 at 256k, 8192 at 1M).
    ...
    if _token_cache_requests:
        # the maximum of what the LIVE engines asked for. Flooring it at the
