...
there is no Level 3. Upstream converged with deterministic truncation, which
...
def unfinished_generation_reason(response: Any) -> str:
    """why this response is not a finished generation, or "".
...
def _call_llm_for_summary(prompt: str | list[dict[str, str]], max_tokens: int,
    ...
    try:
        ...
        # keep the cause. Swallowing it turned "maximum context length" into
...
def _normalized_focus_topic(focus_topic: str, max_chars: int = 160) -> str:
    ...
    the cut is named. An ellipsis said something had been shortened but not
...
class _RouteErrorSlot(threading.local):
    """the most recent route exception on THIS thread."""
...
def _invoke_summary_llm_chain(
    ...
    *,
    ...
    for candidate_model in chain:
        # the caller's deadline is an END TIME for the whole attempt, not a
        ...
        if result is None and route_errors is not None and _LAST_ROUTE_ERROR.error is not None:
            route_errors.append(_LAST_ROUTE_ERROR.error)  # keep the cause for rescue
...
# the fidelity contract (backlog A9). The coverage contract above says what
...
def _build_l1_prompt(
    ...
    # the summary is an INDEX into recoverable provenance. The failure
    ...
    if focus_topic:
        ...
        # focus decides EMPHASIS and ORDER, never coverage. Upstream's version
...
def _build_l2_prompt(
    ...
    if focus_topic:
        markers = " / ".join(f"'{marker}'" for marker in _HISTORICAL_HEADING_MARKERS)
        # same correction as L1: emphasis, not coverage. L2 is the *thinner*
...
# deterministic (L3) truncation removed. See errors.SummaryUnavailableError.


# replies that acknowledge or refuse instead of indexing (audit p05 ES06).
...
# the vocabulary a summariser uses to talk about its OWN refusal. A reply
...
def _is_index_shaped_summary(result: str, source_text: str = "") -> bool:
    ...
    when the SOURCE is available, a reply that opens with refusal wording is
    ...
    for marker in _NON_INDEX_REFUSAL_MARKERS:
        ...
        # rejection is the DANGEROUS direction here: a rejected summary means
...
def summarize_with_escalation(
    ...
    there is no deterministic L3; when every route fails this raises
    ...
    # remember whether a route DID answer but the answer was not shorter
    ...
    route_errors: list[BaseException] = []  # why the routes failed (ES03)

    rejected_as_non_index: list[str] = []  # see ES06

    def _accepts(result: str) -> bool:
        # "smaller than the source" was the ONLY substantive acceptance test,
    ...
    # no deterministic truncation. Every route failed (provider error,
    ...
    # name the actual route failure so the caller's rescue predicate can see
