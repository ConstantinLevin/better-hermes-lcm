...
# private carrier for a recovered body that could not be externalized.
_RECOVERED_BODY_KEY = "_lcm_recovered_body"
# the host's CURRENT home for oversized tool results. Hermes writes them to
...
def _hermes_home_path(hermes_home: str | None = None) -> Path:
    """the host home this process is working against."""
...
def _allowed_persisted_output_dirs(hermes_home: str | None = None) -> list[Path]:
    ...
    the host's spillover directory joins the older temp directory here.
...
def recover_hermes_persisted_output_with_file_stat(
    text: str | None,
    hermes_home: str | None = None,  # which host home to trust (p06 I2)
...
def protect_message_for_ingest(
    ...
    # recovery must use the SAME host home the engine's replay path uses.
    ...
    if recovered_with_stat is not None:
        ...
        if normalized_recovered_content:
            ...
            # the durable copy does not depend on the opt-in generic
    ...
    # if the durable copy could not be written but the bytes ARE in hand,
...
def protect_messages_for_ingest(
    ...
    this list is paired POSITIONALLY with the caller's own messages (the
...
def protect_messages_for_ingest_with_attachments(
    ...
    when the durable copy of a recovered host output could not be written,
