...
# the engine each tool call was handed, visible to helpers that have no
...
def _require_engine(kwargs: Dict[str, Any]) -> "LCMEngine | None":
    ...
    _CURRENT_TOOL_ENGINE.engine = engine
...
def _node_index_block_payload(engine: Any, node: Any, *, max_chars: int = 4_000) -> Dict[str, Any]:
    """the sidecar's multi-line index block for a node result, only when the
    ...
    try:
        ...
        # "this node has no sidecar" and "its sidecar could not be read" are
...
def _node_index_slice_payload(engine: Any, node: Any, *, offset: int = 0,
    ...
    """one page of a node's stored index block, from ``offset``.
    ...
    try:
        meta = store.read(int(node.node_id))
    except Exception as exc:  # see _node_index_block_payload
        logger.warning("LCM could not read node metadata for %s: %s", node.node_id, exc)
        return {
            "index_block_unavailable": True,
            "index_block_error": str(exc)[:200],
...
def _unresolved_node_error(engine: "LCMEngine", node_id, status) -> str:
    """"not found" vs "could not be resolved" (round-2 verify-4 #21).
...
def _get_session_node(engine: "LCMEngine", node_id: int, *, status=None):
    ...
    authorization is REACHABILITY from the current session, not session
...
def _is_reachable_from_current_session(engine: "LCMEngine", node: Any, *,
    ...
    ``status`` receives ``unresolved_reason`` when the answer is "the search
    ...
    for _hop in range(max_hops):
        ...
        for child_id in frontier:
            try:
                # ask for THIS session's parents first, so a node with more
...
def _scaled_cap(base: int, engine: Any = None) -> int:
    """tool response char caps scale with the window (x1 at 256k, x4 at 1M).
...
def _bounded_inspect_json(response: dict[str, Any]) -> str:
    ...
    no response char cap and no field-level cut. Every free-text field
...
def _preanswer_evidence_enabled(engine: "LCMEngine") -> bool:
    """the activation boundary for the pre-answer evidence subsystem.
...
# `_sanitized_tool_calls_for_response` stood here. Raw-row expansion used to omit stored tool
# calls entirely and still answer has_more=false (round-2 verify-4 #20); they are returned now,
# but they went through the compaction argument sanitiser first, which parsed and re-serialised
# any valid JSON argument string. The exact-bytes recovery path therefore returned
# 0.12345678901234568 for a stored 0.123456789012345678901234567890, and a continuation walk
# paged over that transformed copy rather than the row (#56). The stored calls are rendered as
# they are; size is what `_slice_content_for_response` is for, and it pages with a cursor.
...
def _pagination_payload(
    *,
    ...
    has_more: bool,
    tool_calls_offset: int = 0,          # see _expand_message_sources
...
def _expand_message_sources(
    ...
    *,
    ...
    tool_calls_offset: int = 0,  # resume a paged tool-call rendering
    envelope_offset: int = 0,  # resume a paged envelope rendering
    ...
    missing_source_ids: list[int] = []  # sources that could not be read
    budget_used = 0
    ...
    for relative_index, store_id in enumerate(source_ids):
        ...
        if not stored:
            # a source that cannot be read is not a source that was read.
        ...
        if ref:
            ...
            if ref_payload is not None and ref_payload.get("corrupt"):
                # the loader detects a truncated/half-written payload; an
        ...
        # an assistant turn's tool CALLS are part of what it said. Omitting
        ...
        call_slice = None
        # the envelope slice (normal or corrupt) is tracked with the other
        ...
        if stored_tool_calls:
            ...
            if call_slice["content_truncated"]:
                ...
                # the continuation must come back to THIS node with the call offset;
        ...
        # the host fields the columns do not project belong to this row too;
        ...
        if stored.get("envelope_corrupt"):
            # corrupt envelope JSON is not an empty envelope, and the raw
        ...
        if (
            ...
            next_source_offset = source_index
            # a FINISHED body reports next_content_offset 0. Re-using that
            ...
            # a FINISHED field parks its cursor at its own end, exactly like
    ...
    # every cursor travels with the page, or following the continuation
...
def _authorized_child_node(engine: Any, child_id: Any) -> Any:
    """a recorded child of an already-authorized node.
...
def _expand_child_nodes(
    ...
    *,
    ...
    missing_child_ids: list[int] = []  # recorded but no longer present
    for relative_index, child_id in enumerate(selected_source_ids):
        child = _authorized_child_node(engine, child_id)  # follows retained lineage
        if child is None:
...
def _search_reporting_progress(search, query: str, *, progress: Dict[str, Any], **kwargs):
    """call a search that reports completeness, tolerating one that cannot.
...
def _collect_context_blocks_for_node(
    ...
    *,
    ...
    if node.source_type == "messages":
        ...
        # an EMPTY page whose pagination says sources are missing or the
        ...
        # a child block that carries ONLY a failure (a recorded child that
...
def _serialize_loaded_message(
    ...
    *,
    ...
    if row.get("tool_name"):
        item["tool_name"] = row.get("tool_name")
    # the host envelope is part of the row. This serializer returned content
...
def lcm_load_session(args: Dict[str, Any], **kwargs) -> str:
    ...
    # `max_content_chars` is the caller's own argument and is honoured
...
class _RecentIncomplete(Exception):
    """the recent-window scan could not complete. Distinct from an empty
...
def _recent_leaf_sections(
    ...
    returns ``(sections, total_matching)``. The display limit used to be
    ...
    with engine._dag._db_lock:
        connection = engine._dag.connection
        if connection is None:
            # a closed/unavailable DAG is not an empty window (verify-4 #13)
...
def _recent_leaf_sections_staged(
    ...
    connection = engine._dag.connection
    if connection is None:
        raise _RecentIncomplete(  # see above (verify-4 #13)
            "the summary database is not available; this window could not be scanned"
        )
    ...
    try:
        with engine._dag._db_lock:
            ...
            if len(id_rows) > _LCM_RECENT_FRONTIER_WORK_LIMIT:
                ...
                # "too much matched to scan safely" is not "nothing happened".
        ...
        # the work-cap signal must reach the caller. The broad handler below
        ...
        # a failed read is not an empty window either.
    ...
    try:
        ...
        # failing CLOSED here returned an empty, "complete" window over
...
def _bounded_recent_json(response: dict[str, Any], sections: list[dict[str, Any]],
    ...
    response["sections"] = []
    # the number the WINDOW holds, counted before the display limit
    ...
    # NO response char cap, and no cutting inside a section.
    #
...
def lcm_recent(args: Dict[str, Any], **kwargs) -> str:
    ...
    # this is the front end for the TEMPORAL ROLLUP subsystem, which is
    ...
    if fallback:
        ...
        except _RecentIncomplete as exc:        
            sections, total_matching, incomplete_reason = [], 0, str(exc)
        except Exception as exc:                  # a read failure is not an empty window
    ...
    response: dict[str, Any] = {
        ...
        # say plainly when the window could not be scanned
...
def _lcm_grep_full_text(args: Dict[str, Any], **kwargs) -> str:
    ...
    search_failures: list[Dict[str, str]] = []  # see the except blocks below
    bounded_scans: list[Dict[str, Any]] = []  # scans that stopped at a work cap
    more_results_available = False  # exact page, corpus holds more (round-2 verify-2 #11)

    def _scan_note(source_name: str, scan: Dict[str, Any]) -> Dict[str, Any] | None:
        """classify one scan: exact, more-to-come, or actually capped.
    ...
    # if the current turn could not be stored, this search cannot see it.
    ...
    if consecutive_ingest_failures or unarchived_revisions:
        ...
        if unarchived_revisions:
            # a host EDIT that could not be archived means the corrected
    ...
    if content_scope in {"history", "both"}:
        try:
            message_progress: dict[str, Any] = {}  # was this raw scan exhaustive?
            ...
            # a search that FAILED is not a search that found nothing. The
    ...
    if content_scope in {"history", "both"} and session_scope == "current" and not raw_message_filter_active:
        try:
            node_progress: dict[str, Any] = {}  # was this summary scan exhaustive?
    ...
    if searches_externalized and externalized_filter_active:
        ...
        # every match found is delivered. This loop used to stop
    ...
    # say how the raw query was interpreted whenever anything was dropped
    ...
    if search_failures or bounded_scans:
        # a partial, failed or work-capped search must never look like an
    ...
    if more_results_available:
        # an EXACT page over a corpus that holds more matches. This is not a
...
def _lcm_recall_fts_arm(
    ...
    if "error" in payload:
        return [], payload
    # the arm's own completeness travels with its hits. A grep result that
...
def lcm_recall(args: Dict[str, Any], **kwargs) -> str:
    ...
    if run_fts:
        ...
        if fts_error is not None and "error" in fts_error:
            ...
            if fts_error is not None:
                # the arm RAN but not exhaustively; its hits are kept and its
    ...
    for entry in selected_entries:
        ...
        # no response char cap. `limit` is the contract: the caller
    ...
    if detail == "answer_ready":
        ...
        # the response is serialised as it stands. This used to echo
...
def lcm_describe(args: Dict[str, Any], **kwargs) -> str:
    ...
    if node_id is not None:
        ...
        # a truncated index block names THIS call as its continuation, so
        ...
        # a node's OWN summary is pageable here. A truncated child summary
...
def lcm_expand(args: Dict[str, Any], **kwargs) -> str:
    ...
    # the default page is window-weighted (4000 at 256k, 32000 at 1M)
    ...
    hydrate_externalized = raw_hydrate  # node mode returns externalized outputs inline
    ...
    if externalized_ref:
        ...
        if payload.get("corrupt"):
            # the loader detected a truncated/half-written artifact; returning
    ...
    if raw_store_id_arg is not None:
        ...
        # the host fields the columns do not project (name, reasoning
        ...
        if isinstance(stored.get("envelope"), dict) and stored["envelope"]:
            ...
            rendered_envelope = json.dumps(stored["envelope"], ensure_ascii=False, default=str)
            # the budget is spent ONCE, in order: content, then the
            ...
            if envelope_slice["content_truncated"]:
                ...
                result["envelope_continue_with"] = {
                    ...
                    "envelope_offset": envelope_slice["next_content_offset"],
                    # every continuation carries EVERY cursor, or following one restarts
                    # a field the caller already has (round-4 verify-2 #9)
                    "tool_calls_offset": tool_calls_offset,
                }
        elif stored.get("envelope_corrupt"):
            # the raw-store path did not report envelope corruption AT ALL:
        ...
        # an assistant turn's tool CALLS are part of what it said. Node
        ...
        if stored_tool_calls:
            ...
            # the calls are rendered as the row holds them (#56). A historical row
            # with a megabyte of inline base64 in its arguments is bounded by the page budget
            # below, which returns a cursor and says has_more — not by rewriting the bytes the
            # caller asked this tool for.
            rendered_calls = json.dumps(stored_tool_calls, ensure_ascii=False, default=str)
            ...
            if call_slice["content_truncated"]:
                ...
                result["tool_calls_continue_with"] = {
                    ...
                    "tool_calls_offset": call_slice["next_content_offset"],
                    # carry the envelope cursor too (round-4 verify-2 #9). A CORRUPT
    ...
    if node.source_type == "messages":
        messages, pagination = _expand_message_sources(
            ...
            source_limit=source_limit,
            content_offset=content_offset,
            tool_calls_offset=tool_calls_offset,  # resume paged tool calls
            envelope_offset=envelope_offset,  # resume a paged envelope
...
def lcm_expand_query(args: Dict[str, Any], **kwargs) -> str:
    ...
    context_default = max(  # curved (32k at 256k, 125k at 1M)
        max_tokens,
    ...
    # an explicitly requested node that does not resolve must be NAMED. It
    ...
    search_incompleteness: list[dict[str, Any]] = []  # round-3 verify-4 #12
    ...
    if raw_node_ids:
        ...
        # ask BOTH searches how exhaustive they were; query mode reported
        # complete:true over a capped or degraded scan (round-3 verify-4 #12).
        node_progress: Dict[str, Any] = {}
        message_progress: Dict[str, Any] = {}
        # ask for ONE MORE than the caller's limit: requesting exactly the
    ...
    if not nodes and not raw_results:
        ...
        if search_incompleteness:
            # an empty result from a capped or failed scan is not an absence
            # (round-4 verify-4 #13)
            answer = (
    ...
    # a FIELD left unread makes the context truncated too. Only whole-page
    ...
    # everything the caller asked for that this answer does NOT cover
    ...
    # a source row this answer could not read makes the answer incomplete,
    ...
    # completeness is the conjunction of EVERY traversal and hydration
...
def lcm_inspect(args: Dict[str, Any], **kwargs) -> str:
    ...
    fresh_tail_count = max(0, int(engine.effective_fresh_tail_count or 0))  # curved
...
def lcm_status(args: Dict[str, Any], **kwargs) -> str:
    ...
    return json.dumps({
        ...
        "window_scaling": engine.window_scaling_status(),
...
def lcm_doctor(args: Dict[str, Any], **kwargs) -> str:
    ...
    try:
        # BOTH source types, and the population is named. The check walked
    ...
    # index coverage (opt-in: it reads every node's sources)
