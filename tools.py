...
# fork: better-hermes-lcm — the engine each tool call was handed, visible to helpers that have no
...
def _require_engine(kwargs: Dict[str, Any]) -> "LCMEngine | None":
    ...
    _CURRENT_TOOL_ENGINE.engine = engine  # fork: better-hermes-lcm
...
def _node_index_block_payload(engine: Any, node: Any, *, max_chars: int = 4_000) -> Dict[str, Any]:
    """fork: better-hermes-lcm — the sidecar's multi-line index block for a node result, only when the
    ...
    try:
        ...
        # fork: better-hermes-lcm — "this node has no sidecar" and "its sidecar could not be read" are
...
def _node_index_slice_payload(engine: Any, node: Any, *, offset: int = 0,
    ...
    """fork: better-hermes-lcm — one page of a node's stored index block, from ``offset``.
    ...
    try:
        meta = store.read(int(node.node_id))
    except Exception as exc:  # fork: better-hermes-lcm — see _node_index_block_payload
        logger.warning("LCM could not read node metadata for %s: %s", node.node_id, exc)
        return {
            "index_block_unavailable": True,
            "index_block_error": str(exc)[:200],
...
def _unresolved_node_error(engine: "LCMEngine", node_id, status) -> str:
    """fork: better-hermes-lcm — "not found" vs "could not be resolved" (round-2 verify-4 #21).
...
def _get_session_node(engine: "LCMEngine", node_id: int, *, status=None):
    ...
    fork: better-hermes-lcm — authorization is REACHABILITY from the current session, not session
...
def _is_reachable_from_current_session(engine: "LCMEngine", node: Any, *,
    ...
    fork: better-hermes-lcm — ``status`` receives ``unresolved_reason`` when the answer is "the search
    ...
    for _hop in range(max_hops):
        ...
        for child_id in frontier:
            try:
                # fork: better-hermes-lcm — ask for THIS session's parents first, so a node with more
...
def _scaled_cap(base: int, engine: Any = None) -> int:
    """fork: better-hermes-lcm — tool response char caps scale with the window (x1 at 256k, x4 at 1M).
...
def _bounded_inspect_json(response: dict[str, Any]) -> str:
    ...
    fork: better-hermes-lcm — no response char cap and no field-level cut. Every free-text field
...
def _preanswer_evidence_enabled(engine: "LCMEngine") -> bool:
    """fork: better-hermes-lcm — the activation boundary for the pre-answer evidence subsystem.
...
def _sanitized_tool_calls_for_response(tool_calls: Any) -> Any:
    """fork: better-hermes-lcm — tool calls rendered for a reader, with inline payloads marked.
...
def _pagination_payload(
    *,
    ...
    has_more: bool,
    tool_calls_offset: int = 0,          # fork: better-hermes-lcm — see _expand_message_sources
...
def _expand_message_sources(
    ...
    *,
    ...
    tool_calls_offset: int = 0,  # fork: better-hermes-lcm — resume a paged tool-call rendering
    envelope_offset: int = 0,  # fork: better-hermes-lcm — resume a paged envelope rendering
    ...
    missing_source_ids: list[int] = []  # fork: better-hermes-lcm — sources that could not be read
    budget_used = 0
    next_source_offset: int | None = source_offset
    next_content_offset = content_offset
    next_tool_calls_offset = 0
    next_envelope_offset = 0  # fork: better-hermes-lcm — the envelope has a cursor of its own
    corrupt_payload_refs: list[dict[str, Any]] = []  # fork: better-hermes-lcm (round-3 verify-3)
    ...
    for relative_index, store_id in enumerate(source_ids):
        ...
        if not stored:
            # fork: better-hermes-lcm — a source that cannot be read is not a source that was read.
        ...
        if ref:
            ...
            if ref_payload is not None and ref_payload.get("corrupt"):
                # fork: better-hermes-lcm — the loader detects a truncated/half-written payload; an
        ...
        # fork: better-hermes-lcm — an assistant turn's tool CALLS are part of what it said. Omitting
        ...
        call_slice = None
        # fork: better-hermes-lcm — the envelope slice (normal or corrupt) is tracked with the other
        ...
        # fork: better-hermes-lcm — the host fields the columns do not project belong to this row too;
        ...
        if stored.get("envelope_corrupt"):
            # fork: better-hermes-lcm — corrupt envelope JSON is not an empty envelope, and the raw
        ...
        # fork: better-hermes-lcm — the rendered tool calls are charged to the SAME budget. Leaving
        ...
        # fork: better-hermes-lcm — the envelope is charged too, and an unfinished envelope keeps the
        ...
        if (
            ...
            next_source_offset = source_index
            # fork: better-hermes-lcm — a FINISHED body reports next_content_offset 0. Re-using that
            ...
            # fork: better-hermes-lcm — a FINISHED field parks its cursor at its own end, exactly like
    ...
    # fork: better-hermes-lcm — every cursor travels with the page, or following the continuation
    ...
    if missing_source_ids:
        ...
        # fork: better-hermes-lcm — a corrupt externalized payload is not an empty one (round-3 verify-3)
        pagination["complete"] = False
...
def _authorized_child_node(engine: Any, child_id: Any) -> Any:
    """fork: better-hermes-lcm — a recorded child of an already-authorized node.
...
def _expand_child_nodes(
    ...
    *,
        ...
        expanded.append(
            {
                ...
                # fork: better-hermes-lcm — where the REST of this summary is. Pointing at expansion
    ...
    if missing_child_ids:
        # fork: better-hermes-lcm — a recorded child that is no longer in the DAG is reported, never
        ...
        # fork: better-hermes-lcm — say it in the SAME field every other path uses. `incomplete=True`
...
def _search_reporting_progress(search, query: str, *, progress: Dict[str, Any], **kwargs):
    """fork: better-hermes-lcm — call a search that reports completeness, tolerating one that cannot.
...
def _collect_context_blocks_for_node(
    ...
    *,
    ...
    if node.source_type == "messages":
        ...
        # fork: better-hermes-lcm — an EMPTY page whose pagination says sources are missing or the
        ...
        # fork: better-hermes-lcm — a child block that carries ONLY a failure (a recorded child that
...
# fork: better-hermes-lcm — where the synthesis route's termination status is left for the caller.
...
def _synthesize_expansion_answer(
    *,
    ...
    # fork: better-hermes-lcm — a synthesised answer that stopped at the generation limit was returned
...
def _serialize_loaded_message(
    ...
    *,
    ...
    if row.get("tool_name"):
        item["tool_name"] = row.get("tool_name")
    # fork: better-hermes-lcm — the host envelope is part of the row. This serializer returned content
...
def lcm_load_session(args: Dict[str, Any], **kwargs) -> str:
    ...
    # fork: better-hermes-lcm — `max_content_chars` is the caller's own argument and is honoured
...
class _RecentIncomplete(Exception):
    """fork: better-hermes-lcm — the recent-window scan could not complete. Distinct from an empty
...
def _recent_leaf_sections(
    ...
    fork: better-hermes-lcm — returns ``(sections, total_matching)``. The display limit used to be
    ...
    with engine._dag._db_lock:
        connection = engine._dag.connection
        if connection is None:
            # fork: better-hermes-lcm — a closed/unavailable DAG is not an empty window (verify-4 #13)
...
def _recent_leaf_sections_staged(
    ...
    connection = engine._dag.connection
    if connection is None:
        raise _RecentIncomplete(  # fork: better-hermes-lcm — see above (verify-4 #13)
            "the summary database is not available; this window could not be scanned"
        )
    ...
    try:
        with engine._dag._db_lock:
            ...
            if len(id_rows) > _LCM_RECENT_FRONTIER_WORK_LIMIT:
                ...
                # fork: better-hermes-lcm — "too much matched to scan safely" is not "nothing happened".
        ...
        # fork: better-hermes-lcm — the work-cap signal must reach the caller. The broad handler below
    ...
    try:
        ...
        # fork: better-hermes-lcm — failing CLOSED here returned an empty, "complete" window over
...
def _bounded_recent_json(response: dict[str, Any], sections: list[dict[str, Any]],
    ...
    response["sections"] = []
    # fork: better-hermes-lcm — the number the WINDOW holds, counted before the display limit
    ...
    # fork: better-hermes-lcm — NO response char cap, and no cutting inside a section.
    #
...
def lcm_recent(args: Dict[str, Any], **kwargs) -> str:
    ...
    # fork: better-hermes-lcm — this is the front end for the TEMPORAL ROLLUP subsystem, which is
    ...
    if fallback:
        ...
        except _RecentIncomplete as exc:          # fork: better-hermes-lcm
    ...
    response: dict[str, Any] = {
        ...
        # fork: better-hermes-lcm — say plainly when the window could not be scanned
...
def _lcm_grep_full_text(args: Dict[str, Any], **kwargs) -> str:
    ...
    search_failures: list[Dict[str, str]] = []  # fork: better-hermes-lcm — see the except blocks below
    ...
    def _scan_note(source_name: str, scan: Dict[str, Any]) -> Dict[str, Any] | None:
        """fork: better-hermes-lcm — classify one scan: exact, more-to-come, or actually capped.
    ...
    # fork: better-hermes-lcm — if the current turn could not be stored, this search cannot see it.
    ...
    if consecutive_ingest_failures or unarchived_revisions:
        ...
        if unarchived_revisions:
            # fork: better-hermes-lcm — a host EDIT that could not be archived means the corrected
    ...
    if content_scope in {"history", "both"}:
        try:
            ...
            # fork: better-hermes-lcm — a search that FAILED is not a search that found nothing. The
    ...
    if searches_externalized and externalized_filter_active:
        ...
        # fork: better-hermes-lcm — every match found is delivered. This loop used to stop
    ...
    # fork: better-hermes-lcm — say how the raw query was interpreted whenever anything was dropped
    ...
    if search_failures or bounded_scans:
        # fork: better-hermes-lcm — a partial, failed or work-capped search must never look like an
    ...
    if more_results_available:
        # fork: better-hermes-lcm — an EXACT page over a corpus that holds more matches. This is not a
...
def _lcm_recall_fts_arm(
    ...
    if "error" in payload:
        return [], payload
    # fork: better-hermes-lcm — the arm's own completeness travels with its hits. A grep result that
...
def lcm_recall(args: Dict[str, Any], **kwargs) -> str:
    ...
    if run_fts:
        ...
        if fts_error is not None and "error" in fts_error:
            ...
            if fts_error is not None:
                # fork: better-hermes-lcm — the arm RAN but not exhaustively; its hits are kept and its
    ...
    for entry in selected_entries:
        ...
        # fork: better-hermes-lcm — no response char cap. `limit` is the contract: the caller
    ...
    if detail == "answer_ready":
        ...
        # fork: better-hermes-lcm — the response is serialised as it stands. This used to echo
...
def lcm_describe(args: Dict[str, Any], **kwargs) -> str:
    ...
    if node_id is not None:
        ...
        # fork: better-hermes-lcm — a truncated index block names THIS call as its continuation, so
        ...
        # fork: better-hermes-lcm — a node's OWN summary is pageable here. A truncated child summary
...
def lcm_expand(args: Dict[str, Any], **kwargs) -> str:
    ...
    # fork: better-hermes-lcm — the default page is window-weighted (4000 at 256k, 32000 at 1M)
    ...
    if externalized_ref:
        ...
        if payload.get("corrupt"):
            # fork: better-hermes-lcm — the loader detected a truncated/half-written artifact; returning
    ...
    if raw_store_id_arg is not None:
        ...
        # fork: better-hermes-lcm — the host fields the columns do not project (name, reasoning
        ...
        if isinstance(stored.get("envelope"), dict) and stored["envelope"]:
            ...
            rendered_envelope = json.dumps(stored["envelope"], ensure_ascii=False, default=str)
            # fork: better-hermes-lcm — the budget is spent ONCE, in order: content, then the
            ...
            # fork: better-hermes-lcm — the raw-store path did not report envelope corruption AT ALL:
        ...
        # fork: better-hermes-lcm — an assistant turn's tool CALLS are part of what it said. Node
...
def lcm_expand_query(args: Dict[str, Any], **kwargs) -> str:
    ...
    # fork: better-hermes-lcm — an explicitly requested node that does not resolve must be NAMED. It
    ...
    if raw_node_ids:
        ...
        # fork: better-hermes-lcm — ask BOTH searches how exhaustive they were; query mode reported
        # complete:true over a capped or degraded scan (round-3 verify-4 #12).
        node_progress: Dict[str, Any] = {}
        message_progress: Dict[str, Any] = {}
        # fork: better-hermes-lcm — ask for ONE MORE than the caller's limit: requesting exactly the
    ...
    if not nodes and not raw_results:
        ...
        if search_incompleteness:
            # fork: better-hermes-lcm — an empty result from a capped or failed scan is not an absence
            # (round-4 verify-4 #13)
            answer = (
    ...
    # fork: better-hermes-lcm — nodes the caller explicitly asked for that this call will not process
    ...
    # fork: better-hermes-lcm — a FIELD left unread makes the context truncated too. Only whole-page
    ...
    try:
        _LAST_SYNTHESIS_STATUS.unfinished = ""  # fork: better-hermes-lcm — see below
    ...
    # fork: better-hermes-lcm — everything the caller asked for that this answer does NOT cover
    ...
    # fork: better-hermes-lcm — a source row this answer could not read makes the answer incomplete,
    ...
    # fork: better-hermes-lcm — completeness is the conjunction of EVERY traversal and hydration
...
def lcm_status(args: Dict[str, Any], **kwargs) -> str:
    ...
    return json.dumps({
        ...
        "window_scaling": engine.window_scaling_status(),  # fork: better-hermes-lcm
...
def lcm_doctor(args: Dict[str, Any], **kwargs) -> str:
    ...
    try:
        # fork: better-hermes-lcm — BOTH source types, and the population is named. The check walked
    ...
    # fork: better-hermes-lcm — index coverage (opt-in: it reads every node's sources)
