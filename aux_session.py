...
# fork: better-hermes-lcm — how far the host-session ancestry walk follows parent links before it
...
class AuxiliarySessionMixin:
    ...
    def _caller_is_auxiliary_agent_frame(self, caller_self: Any) -> bool:
        ...
        if log_prefix.startswith("[subagent-"):
            return True
        # fork: better-hermes-lcm — a restricted TOOLSET is not an identity. Upstream returned True for
    ...
    def _has_auxiliary_identity(caller_self: object) -> bool:
        """fork: better-hermes-lcm — is this frame identified as an auxiliary/child agent at all?
    ...
    def _is_live_auxiliary_child_session(
        ...
        try:
            ...
            # fork: better-hermes-lcm — say that the answer is unknown rather than merely negative
            logger.warning(
    ...
    def _session_has_auxiliary_ancestor(
        ...
        visited: set[str] = set()
        current = session_id
        # fork: better-hermes-lcm — a bounded or failed walk is UNKNOWN, not a proven negative
