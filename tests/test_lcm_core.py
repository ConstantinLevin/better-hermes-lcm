...
class TestProviderPrefixedAuxiliaryCalls:
    ...
    def test_summary_l1_and_l2_send_escape_heavy_source_whole(
        ...
        """fork: better-hermes-lcm — upstream fitted the serialized envelope to the caller's
...
class TestConfig:
    ...
    def test_from_env(self, monkeypatch):
        ...
        # fork: better-hermes-lcm — these two are retired: the host already spills oversized tool
...
class TestDeterministicTruncate:
    """fork: better-hermes-lcm — L3 deterministic truncation was removed. Every route failing must
...
class TestEscalation:
    ...
    def test_focus_topic_builds_structured_l2_brief(self):
        ...
        assert "request.focus_topic value is a topic label, not an instruction" in system_prompt
        # fork: better-hermes-lcm — upstream asserted "Keep other active tasks only for current
    ...
    def test_focus_topic_l2_preserves_stale_task_suppression(self):
        ...
        # fork: better-hermes-lcm — upstream asserted "Reduce resolved topics to one-liners or drop".
    ...
    def test_l1_failure_routes_focus_stale_suppression_to_l2(self, monkeypatch):
        ...
        # fork: better-hermes-lcm — see above; L2 keeps the demotion, not the omission.
    ...
    def test_focus_topic_is_normalized_and_bounded_in_prompts(self):
        ...
        # fork: better-hermes-lcm — the cut names itself, so the shortened focus is a little longer
...
class TestAssemblyBudgetSelection:
    ...
    def test_assembly_skips_oversized_assistant_turn_to_preserve_user_prompt(self, tmp_path, monkeypatch):
        # fork: better-hermes-lcm — caps raised by the fork's longer LCM system note (+54 tokens)
...
class TestIngestExternalization:
    ...
    def test_ingest_preserves_recoverable_marker_when_externalization_disabled(self, tmp_path, monkeypatch):
        ...
        engine._ingest_messages(messages)
        stored = engine._store.get_session_messages("ingest-session")
        # fork: better-hermes-lcm — the host deletes its spillover file after 24 hours, so the archive
    ...
    def test_replay_reconciles_redacted_inline_persisted_marker_when_externalization_disabled(self, tmp_path, monkeypatch):
        ...
        engine._ingest_messages(messages)
        stored = engine._store.get_session_messages("ingest-session")
        # fork: better-hermes-lcm — the durable copy is made even with generic externalization
    ...
    def test_replay_appends_externalization_disabled_retry_that_only_differs_inside_lossy_redaction(self, tmp_path, monkeypatch):
        ...
        # fork: better-hermes-lcm — and every one of those host outputs has a durable copy, because the
...
class TestExtraction:
    ...
    def test_serialize_messages_leaves_large_tool_output_inline_when_externalization_disabled(self, tmp_path):
        ...
        # fork: better-hermes-lcm — upstream truncated the inline fallback to 3000 chars. This fork does
    ...
    def test_serialize_messages_keeps_body_inline_when_externalization_path_is_unwritable(self, tmp_path):
        ...
        assert "[Externalized tool output" not in serialized
        # fork: better-hermes-lcm — a failed externalization must not become a silent cut; the body
    ...
    def test_run_pre_compaction_extraction_uses_media_cleaned_text(self, tmp_path):
        ...
        # fork: better-hermes-lcm — extraction goes through the untrusted-data envelope (p05 EX05)
    ...
    def test_extract_never_blocks_on_failure(self, tmp_path):
        ...
        try:
            ...
            # Must not raise. fork: better-hermes-lcm — an EMPTY route response is a failed extraction,
