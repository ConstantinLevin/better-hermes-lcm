"""fork: better-hermeslcm — the backup is the last line of defence against loss, so it must never
...
def test_the_suite_cannot_reach_live_storage(tmp_path):
    """fork: better-hermeslcm (audit E, E07) — an engine built without an explicit database_path must
