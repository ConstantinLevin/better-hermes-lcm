...
def test_lcm_recent_is_disabled_without_its_subsystem(recent_parts):
    """with rollups off this used to degrade to "fetch the leaf
...
def test_lcm_recent_returns_whole_sections_bounded_only_by_limit(recent_parts):
    ...
    # no response char cap. The section content is a leaf SUMMARY, an
...
def test_recent_fallback_releases_snapshot_before_later_dag_write(recent_parts):
    ...
    # the helper returns (sections, total_matching): the window's own count
...
def test_recent_fallback_releases_transaction_on_lineage_exception(
    ...
    # a failed read is not an empty window: the helper raises so the caller
