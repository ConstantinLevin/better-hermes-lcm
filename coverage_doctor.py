"""fork: better-hermeslcm — ``lcm_doctor coverage``: the executable definition of "no loss".
...
def index_entities_with_bound(text: str, *, limit: int = 400) -> tuple[List[str], bool]:
    """fork: better-hermeslcm — the entity list AND whether the cap cut it short.
...
def coverage_of(summary_text: str, index_block: str, sources_text: str,
    ...
    # fork: better-hermeslcm — no evidence is NOT full coverage. A node whose sources are missing,
    ...
    return {
        ...
        # fork: better-hermeslcm — the score describes only what was EXAMINED (round-2 verify-4 #34)
...
def session_coverage(engine: Any, session_id: Optional[str] = None, *, limit: int = 200,
    ...
    # fork: better-hermeslcm — three outcomes, never conflated: scored, unscored (no evidence), and
    ...
    return {
        ...
        # fork: better-hermeslcm — nodes whose score covers only part of their sources or entities
