...
# fork: better-hermes-lcm — these ranges decide whether a query goes to the LIKE scan instead of the
...
def required_terms_for_symbol_query(terms: List[str]) -> List[str]:
    ...
    fork: better-hermes-lcm — requiring only the symbol-bearing term left its companions optional, so
...
def terms_with_embedded_dropped_symbols(terms: List[str]) -> List[str]:
    ...
    fork: better-hermes-lcm — a standalone symbol (an emoji on its own) is usually a routing trigger,
...
def contains_index_dropped_symbols(text: str) -> bool:
    ...
    fork: better-hermes-lcm — sanitisation maps them to a separator, so ``flag∀`` and ``flag∃``
...
def describe_query_interpretation(query: str) -> dict:
    """fork: better-hermes-lcm — what this query was actually searched for, and what was dropped.
...
def build_snippet(text: str, terms: List[str], width: int = 80) -> str:
    ...
    if not content:
        return ""
    # fork: better-hermes-lcm — match on the ORIGINAL text. Case folding is not length-preserving
