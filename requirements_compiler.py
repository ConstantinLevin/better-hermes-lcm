...
def _deferred_material_count(refs, contract, *, engine) -> int:
    ...
    fork: better-hermes-lcm — a budget that stopped after twelve references hid a thirteenth saying 20
    ...
    # fork: better-hermes-lcm — when the question NAMES its operands, a deferred reference matters only
    ...
    for store_id in store_ids:
        ...
        # fork: better-hermes-lcm — a NUMBER can be spelled out, and a unit can be named in words:
...
def _finish(result: dict[str, Any], *, started: float) -> dict[str, Any]:
    # fork: better-hermes-lcm — one place where a certificate is refused. Candidates the budgets never
    ...
    if deferred or unparsed:
        ...
        # fork: better-hermes-lcm — refusing coverage has to revoke the CERTIFICATE, not merely one
...
# fork: better-hermes-lcm — verbs that place a clause OUTSIDE the counted event on purpose (buying a
...
def _finite_enumeration(
    ...
    *,
    ...
    certificate = {
        ...
        "material_clauses": 0,
        # fork: better-hermes-lcm — clauses that mention the requested unit but the grammatical filter
    ...
    for raw in ranked_baseline_input[: limits.max_candidates]:
        ...
        if hydrated is not None and hydrated["exact_ref"] not in seen:
            ...
            if len(baseline) >= limits.max_hydrated_candidates:
                break
    # fork: better-hermes-lcm — what this call never looked at, and how much of it matters
    # (round-3 verify-4 #15)
