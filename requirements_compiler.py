...
def _deferred_material_count(refs, contract, *, engine) -> int:
    ...
    a budget that stopped after twelve references hid a thirteenth saying 20
    ...
    # when the question NAMES its operands, a deferred reference matters only
    ...
    for store_id in store_ids:
        ...
        # a NUMBER can be spelled out, and a unit can be named in words:
...
def _finish(result: dict[str, Any], *, started: float) -> dict[str, Any]:
    # one place where a certificate is refused. Candidates the budgets never
    ...
    if deferred or unparsed:
        ...
        # refusing coverage has to revoke the CERTIFICATE, not merely one
...
# verbs that place a clause OUTSIDE the counted event on purpose (buying a
...
def _finite_enumeration(
    ...
    *,
    ...
    certificate = {
        ...
        "material_clauses": 0,
        # clauses that mention the requested unit but the grammatical filter
        ...
        for start, end, clause in _material_clauses(content, contract.requested_unit):
            ...
            if verdict != "event":
                if verdict == "unreadable":
                    certificate["unparsed_unit_clauses"] += 1  # round-3 verify-4 #16
                continue
...
def compile_preanswer_evidence(
    ...
    *,
    ...
    for raw in ranked_baseline_input[: limits.max_candidates]:
        ...
        if hydrated is not None and hydrated["exact_ref"] not in seen:
            ...
            if len(baseline) >= limits.max_hydrated_candidates:
                break
    # what this call never looked at, and how much of it matters
    # (round-3 verify-4 #15)
    ...
    result["metrics"]["deferred_candidates"] = len(deferred_refs)  # round-3 #15
