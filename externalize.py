...
def _tool_call_stub(tool_call_id: str) -> str:
    ...
    fork: better-hermeslcm — only "/" and ":" were replaced, so a call id containing a SPACE (or any
...
def load_externalized_payload(ref: str, *, config, hermes_home: str = "") -> Dict[str, Any] | None:
    ...
    # fork: better-hermeslcm — a parseable file is not a valid payload. A file holding only
...
def maybe_externalize_payload(
    ...
    *,
    ...
    ignore_enabled_flag: bool = False,  # fork: better-hermeslcm — see below
) -> Dict[str, Any] | None:
    ...
    fork: better-hermeslcm — ``ignore_enabled_flag`` is for content that would otherwise be LOST
