...
# fork: better-hermeslcm — the receipt for a bypassed session's dropped messages. It is identified by
...
def bypass_omission_counts(text: str) -> tuple[int, int]:
    ...
    fork: better-hermeslcm — reading both forms makes compaction IDEMPOTENT: compacting an already
...
def compact_bypass_omission_marker(text: str) -> str:
    ...
    fork: better-hermeslcm — the receipt is never removed, but when the budget cannot hold it AND the
...
def is_bypass_omission_marker(message: Any) -> bool:
    """fork: better-hermeslcm — is this the receipt above?"""
...
def elide_text(text: str, cap: int, *, original_chars: int | None = None) -> str:
    ...
    fork: better-hermeslcm — an elision must not swallow an EARLIER receipt. Sanitisation runs first
    ...
    # fork: better-hermeslcm — a marker that CROSSES the cut survived as a fragment ("[LCM: 1000 chars
...
def elide_args(args: str, cap: int) -> str:
    ...
    fork: better-hermeslcm — a cut here must not swallow an EARLIER receipt. Sanitisation runs first
...
def tool_call_fields_note(call: dict) -> str:
    """fork: better-hermeslcm — name what a rendered ``name(arguments)`` call leaves out.
...
def missing_tool_result_stub(tool_call_id: str, *, store_ids: Sequence[int] | None = None,
    ...
    fork: better-hermeslcm — upstream's stub said the result was "in the context summary above", which
...
def unmappable_rows_marker(count: int) -> str:
    ...
    fork: better-hermeslcm — a host truncation marker whose file could not be copied has no row the
...
# fork: better-hermeslcm — EVERY marker spelling this module writes, not only "[LCM:". Inheritance
...
def marker_fragments(text: str) -> List[str]:
    ...
    fork: better-hermeslcm — the fragment is not shortened below its closing bracket. A 300-character
...
def inherited_receipts(summaries: Iterable[str]) -> List[str]:
    ...
    fork: better-hermeslcm — a condensed parent is written by the summariser, which has no obligation
    ...
    for summary in summaries:
        for line in str(summary or "").splitlines():
            ...
            if is_marker_line(stripped):
                ...
                continue
            # fork: better-hermeslcm — a receipt can sit AFTER visible text on the same line (a
...
def minimal_assembly_omission_marker() -> str:
    ...
    fork: better-hermeslcm — when neither the full nor the one-line receipt fits the summary budget,
...
def leading_turns_dropped_marker(dropped: int, roles: List[str]) -> str:
    ...
    fork: better-hermeslcm — a conversation replayed to a provider may not open with an assistant or
...
_INLINE_ENVELOPE_FIELDS = (
    # fork: better-hermeslcm — small fields that change what the CONTENT means. `exit_code`/`error`
...
def envelope_inventory(envelope: dict) -> tuple[dict, list[str]]:
    """fork: better-hermeslcm — the same split as :func:`envelope_summary_suffix`, as data.
...
def envelope_summary_suffix(envelope: dict, *, max_listed: int = 10) -> str:
    """fork: better-hermeslcm — render the small outcome fields, inventory the rest.
...
def acknowledgement_only_marker(content: str) -> str:
    """fork: better-hermeslcm — an acknowledgement-shaped turn removed from the summariser's input.
...
def revision_rows_marker(store_ids: List[int]) -> str:
    ...
    fork: better-hermeslcm — a correction the host made to an already-stored message is archived as
...
def aggregated_inherited_receipt_marker(receipts: List[str], node_ids: List[int]) -> str:
    ...
    fork: better-hermeslcm — a condensed parent inherits its children's receipts verbatim, and four
...
def recovered_body_rows_marker(store_ids: List[int]) -> str:
    ...
    fork: better-hermeslcm — when the durable copy of a recovered host output cannot be written, the
...
def compact_assembly_omission_marker(
    *,
    ...
    fork: better-hermeslcm — the receipt is what tells the reader something is missing, so it must
...
def assembly_omission_marker(
    *,
    ...
    if dropped_internal_turns:
        # fork: better-hermeslcm — active-context cleanup removes assistant turns whose only content
    ...
    if redacted_internal_turns:
        # fork: better-hermeslcm — a turn can be PARTLY internal: the visible text is replayed and the
